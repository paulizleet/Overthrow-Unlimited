...
function COverthrowGameMode:OnGameRulesStateChange()
	...
	if nNewState == DOTA_GAMERULES_STATE_PRE_GAME then
		...
		elseif GetMapName() == "temple_quartet" then
			self.TEAM_KILLS_TO_WIN = 50		
		elseif GetMapName() == "forest_solo_same_heroes" then
			self.TEAM_KILLS_TO_WIN = 25
...
function COverthrowGameMode:OnPlayerRandom(userid, heroid)
	print("well this is working at least")
	local playerName = PlayerResource:GetPlayerName(userid)
	local heroName = PlayerResource:GetSelectedHeroName(heroid)
	UTIL_MessageTextAll( playerName .. " has randomed " .. heroName, 255, 255, 255)
